<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-waves icon="el-icon-circle-plus-outline" type="primary" @click="createSinger">
        添加
      </el-button>
    </div>
    ...
    >
      ...
      <el-table-column label="歌手名" min-width="100px" align="center">
        <template slot-scope="{row}">
          <span> {{ row.name }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="出生日期" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.birth | parseTime('{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="所在地" width="200px" align="center">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" top="1%" style="height:96%;overflow-y: auto;" @open="deletePicFlag=false" @close="checkDelete" >
        ...
        <el-tab-pane label="歌手基本信息" name="first">
          <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="80px" style="width: 400px; margin-left:50px;">
            ...
            <el-form-item label="出生日期" prop="birth">
              <el-date-picker v-model="temp.birth" type="date" placeholder="请选择日期" />
            </el-form-item>
      ...
      export default {
        ...
        pvData: [],
        rules: {
          name: [{ required: true, message: '必须填写歌手名', trigger: 'change' }],
          sex: [{ required: true, message: '必须选择性别', trigger: 'change' }],
          birth: [{ required: true, message: '必须选择出生日期', trigger: 'change' }],
        },
    ...
    methods: {
      ...
      handleUpdate(row) {
        ...
        fetchSingerSong(this.temp.id,"").then(result=>{
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          ...
          if(this.deletePicFlag){
            ...
            updateSinger(upData).then(result => {
              this.deletePicFlag=false//如果点击保存则重置删除标记
      ...
      handleDelete(row, index) {
        ...
        this.$confirm('确认永久删除该歌手吗？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          deleteSinger(idData).then(result=>{
            if(result=="success"){
              this.$notify({
                title: 'Success',
                message: '删除成功！',
